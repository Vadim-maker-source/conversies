...
import { getForumCategories, getForumPostsByCategory } from '@/app/lib/api/forum'
...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  // Получаем все категории чтобы найти нужную
  const categories = await getForumCategories()
  
  // Сначала ищем по slug
  let category = categories.find(cat => cat.slug === categoryParam)
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex bg-gray-100 rounded-xl p-1">
                <Link
                  href={`/forum/${category.slug}?sort=newest${page > 1 ? `&page=${page}` : ''}`}
                ...
                <Link
                  href={`/forum/${category.slug}?sort=popular${page > 1 ? `&page=${page}` : ''}`}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    sort === 'popular' 
                      ? 'bg-white text-blue-600 shadow-sm' 
                      : 'text-gray-600 hover:text-gray-900'
                ...
                <Link
                  href={`/forum/${category.slug}?sort=commented${page > 1 ? `&page=${page}` : ''}`}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    sort === 'commented' 
                      ? 'bg-white text-blue-600 shadow-sm' 
                      : 'text-gray-600 hover:text-gray-900'
                ...
                <div className="px-6 py-8 border-t border-gray-100">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {page > 1 && (
                        <Link
                          href={`/forum/${category.slug}?page=${page - 1}&sort=${sort}`}
                      ...
                      <div className="flex items-center gap-1">
                        {Array.from({ length: Math.min(5, pagination.pages) }, (_, i) => {
                          ...
                          if (pageNum <= pagination.pages) {
                            return (
                              <Link
                                key={pageNum}
                                href={`/forum/${category.slug}?page=${pageNum}&sort=${sort}`}
                        ...
                        {pagination.pages > 5 && page <= pagination.pages - 2 && (
                          <Link
                            href={`/forum/${category.slug}?page=${pagination.pages}&sort=${sort}`}
