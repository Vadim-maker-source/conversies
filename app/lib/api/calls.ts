...
export async function initiateCall(chatId: number, type: 'audio' | 'video') {
  ...
  if (chat) {
    const callerName = `${currentUser.name} ${currentUser.surname}`
...
export async function acceptCall(callId: number) {
  ...
  try {
    ...
    if (call.status === 'ringing') {
      ...
      const existingParticipants = await getCallParticipants(callId)
  
  // Уведомляем каждого существующего участника о новом участнике
  existingParticipants.forEach(async (participant) => {
    if (participant.userId !== currentUser.id) {
      await pusherServer.trigger(`user-${participant.userId}`, 'new-participant', {
        callId,
      ...
      await pusherServer.trigger(`call-${callId}`, 'participant-joined', {
        ...
        allParticipants: updatedCall.participants.map(p => ({
          id: p.userId,
          name: p.user.name,
          surname: p.user.surname,
          avatar: p.user.avatar
